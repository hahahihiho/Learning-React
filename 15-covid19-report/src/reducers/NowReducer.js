...
const sort_data = (chartData) => {
    const len = chartData.누적확진자.length;
    for(let i=len-1; i>=0; i--){
        for(let j=0; j<i; j++){
            if(chartData.누적확진자[i] > chartData.누적확진자[j]){
                swap(chartData.누적확진자,i,j)
                swap(chartData.city_name,i,j)
            }
        }
    }
}

const get_chartData = (response) => {
...
export const covidNowAsync = () => async dispatch => {
    ...
    try {
        ...
        response.data.chartData = get_chartData(response);
        sort_data(response.data.chartData);
